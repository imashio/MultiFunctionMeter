...
#define TCNT_LIM        250 // comment 小さくするとおかしい。回転数表示が小さくなる。
// Merging CNT
#define NOISE_CNT       0
...
int main(void)
{
    ...
    unsigned char*   opening_message2 = "   Firmware Rev.4   ";
...
//                cnt = proc_array[Nmed-1]; // 最大値を暫定カウント値にセット
         
                if( cnt != 0xffff ){
                    // 回転数らしきカウント値がいくつあるかカウントする
                    m = 1;
                    for(n=Nmed-2;n>=0;n++){
                        if( (proc_array[n] > cnt*0.75) && (proc_array[n] < cnt*1.25) ){
                            m++;
                        }else{
                            break;
                        }
                    ...
                    // ノイズパルスのマージ
                    for(n=0;n<Nmed-1;n=n+m){
                        if( proc_array[n] < NOISE_CNT ){
