...
// USB communication ( 0:disble , 1:enable )
#define USB_COM         1
...
const char RxName_0[] PROGMEM = "BS";
const char RxName_1[] PROGMEM = "TC";
const char RxName_2[] PROGMEM = "OP";
const char RxName_3[] PROGMEM = "FP";
const char RxName_4[] PROGMEM = "ET";
const char RxName_5[] PROGMEM = "OT";
const char RxName_6[] PROGMEM = "WT";
...
const char   opening_message_0[] PROGMEM = "Fuel Pump Driver";
const char   opening_message_1[] PROGMEM = "Firmware Rev. 10";
...
volatile unsigned char	data_updated[Ndata];    // Data of packet
...
//    FaceMark_init();
    ...
    // value = eq_grad * dec_ang/maxv + eq_intercept
    ...
    uint16_t	SCALE[7];               // Scaling factor for integer processing
    ...
    for(n=0;n<7;n++){
        ...
        USART_transmit_str(char_buffer);
        strcpy_P(char_buffer,PSTR("Data Logger @ Mazda RX-7 FD3S\n\n\n"));
        USART_transmit_str(char_buffer);
    #endif
...
*/
	...
	while(1){
        ...
        ////// Dummy Defi Link Unit - debug //////
        #if 0 // set '0' to Ignore Compile
        for ( index = 0; index < Ndata; index++ ){
            data_updated[index] = 1;
            for( n = 1; n < 4; n++ ){
                data[index][n] = '3';
        ...
        for ( index = 0; index < Ndata; index++ ){
            ...
            if( data_updated[index] == 1 ){
                ...
                if ( valid_packet[index] == 1 ) {
                    ...
                    // Change dec to ISO
                    value[index] = dec_nrm * (uint16_t)pgm_read_word(&(eq_grad[id])) + (int16_t)pgm_read_word(&(eq_intercept[id]));
                }
        ...
        if( FP_maxmode == 0 ){
            lcd_set_str("DUTY");
            lcd_set_numeric(OCR0B*1.0/0xff*100,3,0,0);
            lcd_set_str("%");
        }else{
            lcd_set_str("=FP MAX=");
        }
        ...
        #if USB_COM // Ignore Compile
        //// USART
        USART_transmit_numeric(rpm,5,0,0);
        USART_transmit_str(",   ");
        USART_transmit_numeric(value[3],3,2,1);         // value[3] ... Boost
        USART_transmit_str("kg/cm2,   ");
        USART_transmit_numeric(OCR0B*1.0/0xff*100,3,0,0);   // Fuel Pump Duty
        USART_transmit_str("%,   ");
        USART_transmit_numeric(FP_Volt,2,1,0);// Fuel Pump Voltage
        USART_transmit_str("V,   ");
        USART_transmit_numeric(value[2],3,2,0);         // value[2] ... Fuel Pressure
        USART_transmit_str("kg/cm2\n");
        #endif
