...
// Number of pulse, Nscale [pulse/rpm]
#define Nscale          1
...
*/
			...
			if ( valid_packet == 1 ) {
				...
				// Change dec to ISO
				value[id] = dec_nrm * eq_grad[id] + eq_intercept[id];
                // end of change
				
                lcd_set_numeric(value[id],Nint[id],Nsig[id]-Nint[id],SIGN[id]);
        ...
        // Tacho Meter
            ...
            if(meas_array[tacho_n] >= 0xffff){          // Too Long Gap Pulse
                ...
                freq = FOSC / Ndiv1 / meas_array[tacho_n] / Nscale;
            }
            ...
            if( ( freq - freq_cur ) < RPM_DIFF/60.0 ){  // Tracking
                ...
                freq_cur = FOSC / Ndiv1 / proc_array[Nmed>>1];
            }
        
            // Calcurate RPM
            rpm = (unsigned long int)( 60.0 * freq_cur );
